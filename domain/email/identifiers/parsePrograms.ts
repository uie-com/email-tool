...
export function createProgramForm(programValues: { [key: string]: string | Date }, programSchema: Settings<string[] | string> = EMAIL_TYPES) {
    ...
    Object.keys(programSchema).forEach((key) => {
        ...
        } else if (typeof key === 'string' && Object.keys(programValues).find((valueKey) => normalizeName(programValues[valueKey] as string).includes(normalizeName(key))) !== undefined) {
...
export function shortenIdentifier(id: string) {
    return id
        ...
        // .replaceAll('Before Break', 'BB')
        // .replaceAll('After Break', 'AB')
