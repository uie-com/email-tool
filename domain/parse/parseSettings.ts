...
import { fillTextVariables, parseVariableName } from "./parseVariables";
import { TestSettingValue, ValueDict } from "../schema";
import { getAllIdentifiers } from "./parsePrograms";
import { SETTINGS, SettingValue } from "../settings/settings";
import { EMAIL_TYPES } from "../settings/emails";

const DEBUG = true;
// Returns a one-level bundle of the final settings for a given set of tags
export const getSettings = (settingsObject: any, identifierDict?: ValueDict): ValueDict => {
    console.log('Getting settings for', identifierDict);
    ...
    console.log('Identifiers as setting values', identifiersAsSettingValues);
...
function addSettingsToBundle(bundle: { [settingName: string]: SettingValue[] }, settings: { [settingName: string]: SettingValue }) {
    if (DEBUG) console.log('Adding settings to bundle', settings);
