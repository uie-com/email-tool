...
export function openPopup(url: string) {
    ...
    const popupLeft = (screenWidth) / 2;

    console.log(`Opening popup on ${isLeft ? 'left' : 'right'} at ${popupLeft}px of ${screenWidth}px with ${popupWidth} width, and ${0}px y of ${window.screen.availHeight}px at ${popupHeight}`);
