...
import { fetchAirtableData } from "../integrations/airtable/airtableActions";
import { normalizeName } from "../variables/normalize";
import { Variable, Variables } from "../variables/variableCollection";
...
export class Values {
    ...
    hasValueOf(value: any): boolean {
        ...
        return this.initialValues.find((a) => a.initialValues.find((v) => normalizeName(v.value) === value)) !== undefined;
    }

    hasValueForOf(key: string, value: any): boolean {
        key = this._resolveKey(key);
        value = this._resolveKey(value);
        return this.initialValues.find((v) => v.key === key)?.initialValues.find((v) => normalizeName(v.value) === value) !== undefined;
    }
    ...
    _resolveKey(key: string) {
        ...
        return normalizeName(new Variables(key, [key]).resolveWith(this));
    }
