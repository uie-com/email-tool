...
import { getAllPossibleEmailTypes } from "@/domain/email/identifiers/parsePrograms";
import { listAllTemplates } from "@/domain/email/templates/templates";
...
import { Accordion, Badge, Box, Code, Flex, ScrollArea, Text } from "@mantine/core";
    ...
    const variableOccurance: VariableOccurance = useMemo(() => {
        ...
        templates.forEach((template) => {
            ...
            templateVariables.variables.forEach((variable) => {
                ...
                dict[variable.key].programUses.push(Object.keys(PROGRAM_COLORS).find((program) => normalizeName(template.path).includes(normalizeName(program))) ?? '');
            });
        });
